...
## 整改落地说明（2026-08-28 追记）

上述修复建议已在 `fix/2026-08-27-audit-remediation` 分支落地（含随后的两轮"对修复本身"的双盲复审）。与本报告原方案有出入或有意缩小范围的项，记录如下，以免本报告被当作最终实现的描述误读：

- **P1-11（代理无鉴权）**：实际采用"拒绝携带浏览器 `Origin` 头的请求"的中间件（`proxy/server.rs::reject_browser_origin_requests`），而非本报告建议的本机 token——token 会破坏无法配置请求头的 CLI 客户端。该方案封死浏览器 CSRF 面（含 `Origin: null`）；本机非浏览器进程仍不鉴权，属有意接受的剩余风险。
- **P1-6（>32MB rollout）**：会话*列表*与用量解析已改为有界流式读取，无整文件门槛；但 `load_messages`（点开会话加载全部消息）**有意保留** 32MB 门槛——其输出（完整消息数组）需一次性跨 IPC 传给渲染层，与读取方式无关地随文件大小增长，去掉门槛会把"点不开"换成"界面冻结"。彻底解除需要消息分页，列为后续功能项。
- **P1-12（CI 私钥 environment 门禁）**：test workflow 已彻底移除签名密钥（用 `--config` overlay 关闭 updater 产物），candidate/release 各 job 补齐 `environment: release`。注意 `environment:` 的实际拦截力取决于仓库 Settings → Environments 中是否配置了保护规则——这一步只能由仓库管理员在 GitHub 网页端完成，代码无法自检。
