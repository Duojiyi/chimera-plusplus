...
"#
    ...
    let auth_value: serde_json::Value =
        read_json_file(&chimera_plus_plus_lib::get_codex_auth_path()).expect("read auth.json");
    assert_eq!(
    ...
    let config_text = std::fs::read_to_string(chimera_plus_plus_lib::get_codex_config_path())
...
"#
    ...
    let live_config = std::fs::read_to_string(chimera_plus_plus_lib::get_codex_config_path())
        .expect("read config.toml");
...
"#;
    ...
    let auth_after_switch: serde_json::Value =
        read_json_file(&chimera_plus_plus_lib::get_codex_auth_path())
    ...
    let restored_auth: serde_json::Value =
        read_json_file(&chimera_plus_plus_lib::get_codex_auth_path()).expect("read restored auth");
    assert_eq!(
        restored_auth, oauth_auth,
        "disabling takeover should restore without replacing OAuth auth.json"
    );

    let restored_config = std::fs::read_to_string(chimera_plus_plus_lib::get_codex_config_path())
...
fn reapply_codex_official_live_resyncs_mcp_servers() {
    ...
    let live = std::fs::read_to_string(chimera_plus_plus_lib::get_codex_config_path())
    ...
    let reapplied = chimera_plus_plus_lib::reapply_current_codex_official_live(&state)
...
fn reapply_codex_official_live_projects_mcp_despite_broken_claude_json() {
    ...
    let claude_json = chimera_plus_plus_lib::get_claude_mcp_path();
...
fn sync_all_enabled_reports_broken_app_but_projects_the_rest() {
    ...
    let err = chimera_plus_plus_lib::McpService::sync_all_enabled(&state)
...
fn provider_service_switch_codex_official_accounts_write_auth_json() {
    ...
    let auth_b: serde_json::Value =
        read_json_file(&chimera_plus_plus_lib::get_codex_auth_path()).expect("read auth B");
    assert_eq!(
        auth_b
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
    ...
    let auth_a: serde_json::Value =
        read_json_file(&chimera_plus_plus_lib::get_codex_auth_path()).expect("read auth A");
    assert_eq!(
        auth_a
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
...
"#;
    ...
    let live_after = std::fs::read_to_string(chimera_plus_plus_lib::get_codex_config_path())
        .expect("read config.toml after switch");
    assert!(
