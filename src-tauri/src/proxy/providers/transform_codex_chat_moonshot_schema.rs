...
pub fn wrap_ref_siblings(schema: &mut Value) -> usize {
    ...
    let mut rewritten = 0;
    if map.len() > 1
        && map.get("$ref").is_some_and(Value::is_string)
        && map.get("allOf").is_none_or(Value::is_array)
    {
...
mod tests {
    #[test]
    fn null_and_malformed_schema_values_are_not_overwritten() {
        for original in [
            json!(null),
            json!(false),
            json!({"$ref":null,"description":"keep"}),
            json!({"$ref":"#/$defs/A","allOf":null}),
            json!({"$ref":"#/$defs/A","allOf":{"type":"string"}}),
        ] {
        ...
        for mut body in [
            json!(null),
            json!({"tools":null}),
            json!({"tools":[null,{"function":null},{"function":{"parameters":null}}]}),
        ] {
            assert_eq!(wrap_ref_siblings_in_chat_tools(&mut body), 0);
        }
        assert!(!upstream_requires_ref_sibling_all_of("not a URL"));
