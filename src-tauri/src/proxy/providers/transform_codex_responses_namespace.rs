...
mod tests {
    use super::*;
    use crate::proxy::providers::transform_codex_responses_xai_sanitize::create_xai_native_responses_sse_stream;
    use bytes::Bytes;
    ...
    async fn sse_stream_restores_function_call_events_and_passes_others_through() {
        ...
        let out = create_xai_native_responses_sse_stream(input, map);
        futures::pin_mut!(out);
